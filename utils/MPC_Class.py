...
class MPC_Class:
    ...
    def Data_I_Itertations_init(self):
        ...
        #self.I["nonfixed_Ts_satisfied_u_indices_iter"][-1]=self.I["nonfixed_u_indices_iter"][-1][np.array([0,-1])]  !!!!!was this on IDETC cinference
        self.I["nonfixed_Ts_satisfied_u_indices_iter"][-1]=self.I["nonfixed_Ts_satisfied_u_indices_iter"][-2].copy()
    ...
    def Extraxt_Data_at_each_iter(self,\
        ...
        t_fine_mesh=np.linspace(tau_iters_All_nodes_unscaled_vec[0],tau_iters_All_nodes_unscaled_vec[-1],20*len(tau_iters_All_nodes_unscaled_vec)) #was 20
        ...
        if self.Model_data["plot_iter_flag"]==True: 
            ...
            #plt.show()
            self.ax_state.figure.canvas.draw()  # Ensure self.ax_state's figure is updated
            self.ax_control.figure.canvas.draw()  # Ensure self.ax_control's figure is updated
    ...
    def ODE_Simulation(self,iter_num,tau_iters_All_nodes_unscaled_vec,states_all_matrix,u_extended_matrix_iter,states_all_matrix_unscaled,u_extended_matrix_iter_unscaled):
        ...
        for enum_control in np.arange(self.I["num_u"]):
            U_interp_func[enum_control]=interp1d(np.round(tau_iters_All_nodes_unscaled_vec,6),u_extended_matrix_iter_unscaled[:,enum_control],kind='previous',fill_value="extrapolate")
    ...
    def Save_Result(self):
        ...
        Lagrange_User_Func=self.Functions["Lagrange_User_Func"]
        time_h=Result_data["I"]["time_h"]
        lagrange_cost_val=np.trapz(Lagrange_User_Func(time_h,Result_data["I"]["s_unsc_matrix"],Result_data["I"]["u_unsc_matrix"],self.Model_data),time_h)
        Meyr_Cost_val=0
        Meyr_Cost_val+=self.Functions["User_Mayer_Cost"](self.Model_data["t0"],Result_data["I"]["s_unsc_matrix"],self.Model_data)
        Meyr_Cost_val+=self.Functions["User_Mayer_Cost"](self.Model_data["tf"],Result_data["I"]["s_unsc_matrix"],self.Model_data)
        self.objective_value=lagrange_cost_val+Meyr_Cost_val

        Result_data["objective_value"]=self.objective_value
